...
export async function GET(request: NextRequest) {
  ...
  let query = supabaseAdmin
    ...
    .select("id, job_id, full_name, job_title, job_company_name, location_name, total_experience_months, enrichment_status, linkedin_url, import_order, created_at, updated_at, jobs(title)", { count: "exact" })
  ...
  const { data: counts } = await supabaseAdmin.from("juicebox_profiles").select("enrichment_status")
